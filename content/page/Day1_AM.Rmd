...
`argument  ` - An option or input that you pass to a function. They determine how a function behaves. The argument `skip = 1` tells the the `read_csv()` function to ignore the first row when reading in a data file. To see the default values for a function you can type `?read_csv` in the console.  
