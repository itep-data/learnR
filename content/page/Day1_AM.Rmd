...
Try running some of these examples in your R console. Find new ways to create errors. The more broken the better!
...
The first step of a good scrap audit is reading/loading some data to figure out where all the scrap is going. Here is a small dataset showing the scrap from Jakku. It was salvaged from a crash site and the transfer was incomplete.
...
<div class="data">
__NOTE:__ When your project is open, RStudio sets the working directory to your project folder. When reading in files from a folder outside of your project, you'll need to use the full file path location. For example: `C://Programs/Air_Quality/Ozone_data.csv`
</div>
...
What if we want to ignore the first row of data? 
...
Other types of data have wierd last rows that are a _subtotal_ or just report "END OF DATA". Sometimes we want `read_csv` to ignore the last row, or only pull in a million lines because you don't want to bog down the memory on an old laptop. 
...
A shorthand to write this would be `feed_creatures("small fish", "vegetables", "lichen")`. This works becuase all the arguments remain in the default order.
...
### <i class="fas fa-user-astronaut" aria-hidden="true" style="color:#040707;"></i> Pro-tip! {-}
<div class="well">
One shortcut to see all the arguments for a function is to enter its name in the console followed by a parenthesis, such as ` read_csv( `. Now press `TAB` on the keyboard. This will bring up a drop-down menu of all the available arguments.
</div>
...
Well, well, well. It looks like there is an **All** category we should look into more. Either there's a town hogging all the scrap or we got a bit of cleaning to do.
