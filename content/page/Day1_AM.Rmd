...
We are  __Kristie__ & __Dorian__. Feel free to call us Kristie and Dory if you're feeling casual.   
...
We are going to make this graphic, and you are going to make your name really __BIG__.

![](../images/name_cloud.png){width="65%"}

1. Open R Studio

![](../images/rstudio_open.png){width="35%"}

<br>

2. Copy the script below into your console in R Studio. Start with the line `install.packages` and end with the line `color = 'random-dark')`.
...
![](../images/starwars_group.jpg){align="right" style="width: 54%; margin-left: 26px;"}
...
# Without quotes, R looks for an object called Chewbacca, 
# and then lets you know that it couldn't find one. 
```
...
Let's make a data frame with two columns to hold the characters and their ages from above.
```{r table}
...
- Create the same table above with a 3rd column showing the character's father's names: `c("Darth", "Darth", "Unknown")`
...
Now that you know what objects are and how to create them, let's learn how to use them in _functions_. _Functions_ take one or more inputs called _arguments_. They perform steps based on the _arguments_ and usually return an output object. 
...
The `sum()` function takes the _starwars_ages_ vector as input, performs sum calculations, and returns a single numeric object. Note that we assigned the output object to the name `ages_sum`. If we don't assign the output object it will be printed to the console and won't be saved. Sometimes that's what we want, especially when we're exploring the data. 
...
![](../images/readr-hexbin-sticker-from-rstudio.png){align="left" width="145" style="margin-right:18px;"}
...
What if we want `read_csv` to ignore the first row of data? Let's look through the help window and try to find an argument that can help us. The `skip` argument looks like it could be helpful. Sure enough, the description is exactly what we're looking for here. The default is `skip = 0` (read every line), but we can skip the first line by providing `skip = 1`.
...
###  Limit the number of rows you pull in {-}

Other types of data have wierd last rows that are a _subtotal_ or just report "END OF DATA". Sometimes we want `read_csv` to ignore the last row, or only pull in a million lines because you are worried about space on your c drive. Let's look through the help window and try to find an argument that can help us. The `n_max` argument looks like it could be helpful. Sure enough, the description is exactly what we're looking for here. The default is `n_max = Inf` (read every line), but we can limit the lines we pull in to only a million by `n_max = 1000000`.

```{r, eval = F}

read_csv("https://itep-r.netlify.com/data/starwars_scrap_jakku.csv", skip = 1, n_max = 1000000)
...
|`argument  `| - These are options or inputs that you pass to a function that determine how it behaves. The argument `skip = 1` tells the the `read_csv()` function to ignore the first row when reading in a data file. To see the default values for a function you can type `?read_csv` in the console.  |
