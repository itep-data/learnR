...
Arrange the data in descending order of pounds so that Rey knows what the highest amount of scrap was. Then she can sell the scrap, buy parts for a Starfighter and _we are free to FLY TO ENDOR for our next mission_!
...
### Let's look at the dataset. Remember the different ways? {-}

Hint: `summary()`, `glimpse()`
...
__I see that there are more analytes than we need. We only want to know about `magentic_field` data, so let's filter to that analyte.__ 
...
Let's make a calendar plot to see if we can see when higher magnetic field values occur. Maybe we can pinpoint when magnetic storms are happening.

There is a calendar plot function built into the openair package. We load the openair package, and use `calendarplot()` to make a pretty plot. We hope!
...
It looks like we had really high magentic field readings in November, galactic year 9917. 
