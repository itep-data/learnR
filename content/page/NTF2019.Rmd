...
Let's launch ourselves into the unknown and use R to introduce ourselves. Consider this an RStudio warm-up. We'll use R to introduce ourselves and the data we love _(or don't)_.
...
You can assign values to new objects using the 
...
__Create a table AKA a `data.frame()`__
```{r, intros_tbl}
...
To complete the challenge we'll follow a general roadmap. 

### 7 step data analysis
...
1. __PLOT more__ - _View the data closer_
1. __SUMMARIZE__ the data
1. __SAVE__ the results
...
PPM! That explains the tiny results. Looks like we might need to fix the units on some of these ozone values.
...
(Insert the dplyr intro -> the 6 function toolbox)
...
![](../images/starwars_lunch.jpg){style="width: 53%; align: center;"}
...
![](../images/lubridate.png){style="width: 22%; float: left; margin-right: 30px; margin-top: 8px;"}
...
__Here are some of the functions:__
...
_I'm very literal so I named the data **`starwars_data`**._
...
6. Sometimes a character will not have a specific attribute. We learned earlier how R stores nulls as `NA`. So, some of your filter statements might be `is.na(hair_color)` or `is.na(mass)`. 
...
## Congratulations! 
You've added some great tools to your data analysis tool belt. 
...
ERROR message? Or something behaving strangely and don't know why? See the [Help!](help.html) page for some troubleshooting options.
