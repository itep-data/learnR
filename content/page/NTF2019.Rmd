...
air_data <- mutate(air_data, 
                   ozone_ppb = ozone * 1000) 
```



![](../images/chewie_box.png){style="width: 58%; float: left; margin-right: 20px;"}
...
## We'll use "ymd_hms()" because our dates are written as year-month-day hour_minute_second
...
For our data, we have two monitoring sites. Let's start by looking at the data for one of the sites.
```{r, polar_sites}
...
Sometimes with air data we want to know if there is seasonality in the data. Calendar plots are great for this. The plot below has quite a few layers added to it, but once you have the script it is easy to reuse. You can change the column names to your math your own data, and voila, you have a calendar of your own!
...
> We can look at our data for each day of the week in 2015.
...
> Lastly, let's compare by day of the week and hour of the day.
...
Having an exact record of what you did can serve as great documentation for yourself and others. So next year when your coworker asks why they're getting a different answer for the average ozone concentration, or why you dropped some values from the data, you'll be able to tell them. 

This is also handy when the lab gets back to you and lets you know the ozone observations were okay on the day that the instrument recorded really high temperatures. You can then update that one line in the script and push re-run to remake all of your charts.
